...
def _get_events(STACK_NAME):
    logger.info(F"CF Timestamp        ResourceType  LogicalResourceId   ----> ResourceStatus")
    logger.info("------------------------------------------------------------------------")
    for _ in cloudformation.describe_stack_events(StackName=STACK_NAME)['StackEvents']:
        Timestamp = str(_['Timestamp'])
        logger.info(F"{Timestamp.split('.')[0]} {_['ResourceType']}  {_['LogicalResourceId']}   ----> {_['ResourceStatus']}")
    logger.info("\n")
...
def _waiter(command, STACK_NAME):
    ...
    waiterthread = threading.Thread(target=wait)
...
def delete_cf_stack(STACK_NAME: str):
    ...
    StackId=cloudformation.describe_stack_events(StackName='ystack')['StackEvents'][0]['StackId']
